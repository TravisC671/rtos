// FreeRTOSConfig.h -- Local override for sd_driver (RISC-V / MicroBlaze V)
//
// Based on the FreeRTOS_640x400_RISCV template, adapted for the SD driver.
// Uses heap scheme 6 (newlib integration) so that malloc/pvPortMalloc
// allocate from DDR2 RAM (0x08000000).  This is required because the
// SDMA engine's AXI master can only access the 128 MB DDR2 region.
...
/* HEAP SCHEME 6: newlib integration via heap_useNewlib.c.
   malloc/sbrk allocates from DDR2 (0x08000000) using
   __HeapStart/__HeapTop symbols defined in the linker script. */
