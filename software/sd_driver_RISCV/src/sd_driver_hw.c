...
void sd_hw_clear_interrupts(QueueHandle_t drain_queue)
{
    ...
    // On MicroBlaze V a level-triggered IRQ re-pends immediately after
    // clearing, so the ISR fires synchronously before we continue.
...
// -----------------------------------------------------------------------
// Burst write: 8 words per group via individual stores to the same
...
// -----------------------------------------------------------------------
// Burst read: 8 words per group via individual loads from the same
// device address.
// -----------------------------------------------------------------------
