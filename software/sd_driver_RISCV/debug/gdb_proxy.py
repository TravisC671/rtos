...
# hw_server reports more registers than GDB expects (e.g. FP regs).
# GDB expects 168 bytes (42 regs × 4) for rv32i; hw_server returns
# 272 bytes (68 regs × 4).  Truncate the 'g' response to match.
G_PACKET_MAX_BYTES = 168   # 336 hex chars in the response body
...
def truncate_g_response(token):
    """Truncate an oversized 'g' (read registers) response.

    hw_server returns more registers than GDB expects (e.g. FP regs
    on a target GDB thinks is rv32i).  Truncate the hex data to
    G_PACKET_MAX_BYTES and recompute the checksum.
    """
    body = token[1:-3]           # strip $ and #xx
    max_hex = G_PACKET_MAX_BYTES * 2
    if len(body) > max_hex:
        body = body[:max_hex]
        checksum = sum(body) & 0xFF
        token = b'$' + body + b'#' + f'{checksum:02x}'.encode('ascii')
        log(f"Truncated 'g' response to {G_PACKET_MAX_BYTES} bytes")
    return token
...
def proxy_connection(client_sock, server_sock):
    ...
    # Track 'g' (read registers) requests so we can truncate oversized responses
                    ...
                    for token, is_packet in tokens:
                        if not is_packet:
                            ...
                            if pattern is not None:
                                ...
                                # Track 'g' packets for response truncation
                                if token == b'$g#67':
                    ...
                    for token, is_packet in tokens:
                        if not is_packet:
                            ...
                            if absorb_noack_response:
                                ...
                                # Truncate oversized 'g' responses
                                if g_packet_pending:
                                    g_packet_pending = False
                                    token = truncate_g_response(token)
                                client_sock.sendall(token)
