...
//
// Read and write operations use a two-phase state machine:
// the driver task issues the SD command, returns to the event loop
// to wait for XFER_COMPLETE, then completes the data transfer.
...
//
// CMD0 → CMD8 → CMD55/ACMD41 loop → CMD2 → CMD3 → CMD9 → CMD7 → ACMD6
//
...
static void start_dma_write(void)
{
    sd_hw_clear_interrupts(sd_event_queue);
    op_chunk = op_remaining;
    // Flush dcache so SDMA reads current data from DDR
...
static void handle_dma_read_wait(uint8_t evt)
{
    ...
    case SD_EVT_XFER_COMPLETE:
        // Invalidate dcache so CPU reads fresh data written by SDMA
...
static void sd_driver_task(void *pvParameters)
{
    ...
    if (!sd_card_init()) {
        ...
        if (sd_try_high_speed()) {
            // freq_select=1: sd_clk = 100 MHz / (2 x 1) = 50 MHz, DTCV=0xE
            REG32(SD_CLOCK_CTRL) = 0x000E0105;
        } else {
            // freq_select=2: sd_clk = 100 MHz / (2 x 2) = 25 MHz, DTCV=0xE
            REG32(SD_CLOCK_CTRL) = 0x000E0205;
        }
        vTaskDelay(pdMS_TO_TICKS(1));
        sd_hw_clear_interrupts(sd_event_queue);
